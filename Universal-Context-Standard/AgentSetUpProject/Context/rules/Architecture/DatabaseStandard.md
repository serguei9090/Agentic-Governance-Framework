...
*   **Schema as Code:** DB structure is defined ONLY in `[DB_SCHEMA_FILE]`. Manual SQL changes are strictly forbidden.
...
1.  **Edit Schema:** Modify `[DB_SCHEMA_FILE]`.
2.  **Generate Migration:** Run `[EXECUTE_CMD] [ORM_CLI] migrate dev --name <kebab-case-desc>`.
3.  **Review SQL:** Check the generated SQL file for destructive actions (DROP).
4.  **Update Client:** Run `[EXECUTE_CMD] [ORM_CLI] generate` to update types.
5.  **Commit:** `[DB_SCHEMA_FILE]` AND the migration folder MUST be committed.

## 3. Directory & Naming
*   **Schema:** `[DB_SCHEMA_DIR]/[DB_SCHEMA_FILE]`.
