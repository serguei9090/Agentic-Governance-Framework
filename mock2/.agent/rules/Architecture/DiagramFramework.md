...
## **4. ProDoc Integration (Relations)**

This architecture framework works in tandem with **`rules/ProDoc/relations.md`**.

*   **Consistency:** The nodes and services identified in your diagrams MUST match the terminology used in `relations.md`.
*   **Dependency Mapping:** Use `relations.md` to confirm the "Feature to Service" mappings before drawing arrows. If `relations.md` states that *Feature A* depends on *Service B*, your diagram MUST show an arrow `Feature A --> Service B`.
*   **Truth Source:** `relations.md` is the logical map; `DiagramFramework.md` is the visual representation of that map.
...
2. Grouping: You MUST use `subgraph` to group logical clusters (e.g., `subgraph Cloud`, `subgraph Database_Layer`).
